...
export default async function PilihSkenario({ params }: { params: Promise<{ programId: string }> }) {
  ...
  return (
    <main className="p-6">
      <Link href="/belajar" className="inline-flex items-center text-gray-400 text-sm font-bold mb-6 hover:text-white">← Kembali</Link>
      <h1 className="text-2xl font-black mb-2">Pilih Skenario</h1>
      <p className="text-gray-400 text-sm mb-8">Pilih situasi latihan untuk program {programId}</p>
      <div className="flex flex-col gap-4">
        {scenarios.map((scen) => (
          <Link key={scen.id} href={"/exercise/" + scen.id + "?lang=" + programId}
            className="bg-white/5 border border-white/10 rounded-2xl p-4 flex items-center justify-between active:scale-95 transition-transform hover:border-white/20">
            <div className="flex items-center gap-4">
              <div className="w-10 h-10 bg-black/50 rounded-xl flex items-center justify-center text-xl">{scen.icon}</div>
              <div>
